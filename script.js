...
function displayIssues(issues) {
    ...
    grid.innerHTML = issues.map(issue => `
        <div onclick="openModal('${issue.id}')" class="card bg-white shadow-md cursor-pointer hover:shadow-lg transition-shadow p-5 ${issue.status === 'open' ? 'card-open' : 'card-closed'}">
